...
def read_json_to_string(path):
    ...
    """
    # Open the JSON file located at the specified path
    with open(path, 'r') as file:
        # Load the content of the JSON file into a Python object
        data = json.load(file)
    
    # Convert the Python object back into a JSON-formatted string
    content = json.dumps(data["content"], indent=4)
    return content

def metadata_func(record, default_metadata=None):
    # print("source", record.get("content"))
    return {
